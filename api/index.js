...
const dbGet = async (sql, p=[]) => (await q(sql,p)).rows[0] || null;
const dbAll = async (sql, p=[]) => (await q(sql,p)).rows;
const dbRun = async (sql, p=[]) => (await q(sql,p)).rows[0]?.id || null;

// ── DB init (cached per serverless instance) ─────────────────
let ready = false;
async function ensureDB() {
  ...
  await q(`CREATE TABLE IF NOT EXISTS users (
    ...
    role TEXT NOT NULL DEFAULT 'customer'
      CHECK(role IN ('super_admin','business_owner','staff','customer')),
    is_active INTEGER NOT NULL DEFAULT 1,
    created_at TIMESTAMPTZ DEFAULT NOW())`);
  await q(`CREATE TABLE IF NOT EXISTS businesses (
    id SERIAL PRIMARY KEY, owner_id INTEGER NOT NULL REFERENCES users(id),
    name TEXT NOT NULL, slug TEXT UNIQUE NOT NULL,
    description TEXT, category TEXT, phone TEXT, email TEXT, address TEXT,
  ...
  await q(`CREATE TABLE IF NOT EXISTS staff (
    ...
    name TEXT NOT NULL, title TEXT, bio TEXT, email TEXT, phone TEXT,
    is_active INTEGER NOT NULL DEFAULT 1, created_at TIMESTAMPTZ DEFAULT NOW())`);
  ...
  if (parseInt(row.c) === 0) await seed();
  ready = true;
}
...
async function seed() {
  ...
  const ins = async (sql, p) => (await q(sql,p)).rows[0]?.id;

  await ins(`INSERT INTO users(name,email,password,role) VALUES($1,$2,$3,$4) RETURNING id`,
    ['Super Admin','admin@smartbook.com',h('admin123'),'super_admin']);

  // Salon
  const so = await ins(`INSERT INTO users(name,email,password,role) VALUES($1,$2,$3,$4) RETURNING id`,
    ['Sarah Thompson','sara@citysalon.com',h('owner123'),'business_owner']);
  const s1 = await ins(`INSERT INTO businesses(owner_id,name,slug,description,category,phone,email,address) VALUES($1,$2,$3,$4,$5,$6,$7,$8) RETURNING id`,
  ...
  for (const [n,d,c,t,p] of [
    ['Haircut & Style','Professional cut and blow-dry','Hair',60,35],
    ['Hair Colour','Full colour, highlights and balayage','Hair',120,85],
    ['Keratin Treatment','Smoothing treatment for frizz-free hair','Hair',180,140],
    ['Hair Spa','Deep conditioning treatment','Hair',90,55],
    ['Blow Dry & Styling','Professional blow-dry and finish','Hair',45,25],
    ['Bridal Hair Package','Complete bridal hair styling','Hair',240,280],
    ['Facial','Deep cleansing facial','Skin',60,45],
    ['Gold Facial','Luxurious 24K gold facial','Skin',75,80],
    ['Manicure & Pedicure','Complete nail care with gel polish','Nails',90,40],
    ['Full Body Waxing','Professional waxing service','Skin',90,50],
  ]) await ins(`INSERT INTO services(business_id,name,description,category,duration_min,price) VALUES($1,$2,$3,$4,$5,$6) RETURNING id`,[s1,n,d,c,t,p]);
  await ins(`INSERT INTO staff(business_id,name,title,bio) VALUES($1,$2,$3,$4) RETURNING id`,[s1,'Zara Bennett','Master Stylist','15 years experience in cuts and colour.']);
  await ins(`INSERT INTO staff(business_id,name,title,bio) VALUES($1,$2,$3,$4) RETURNING id`,[s1,'Rachel Mitchell','Colour Specialist','Expert in balayage and highlights.']);
  await ins(`INSERT INTO staff(business_id,name,title,bio) VALUES($1,$2,$3,$4) RETURNING id`,[s1,'Nadia Hussain','Beauty Therapist','Certified aesthetician.']);

  // Dental
  const do2 = await ins(`INSERT INTO users(name,email,password,role) VALUES($1,$2,$3,$4) RETURNING id`,
    ['Dr. James Whitfield','khalid@smileclinic.com',h('owner123'),'business_owner']);
  const s2 = await ins(`INSERT INTO businesses(owner_id,name,slug,description,category,phone,email,address) VALUES($1,$2,$3,$4,$5,$6,$7,$8) RETURNING id`,
    [do2,'Smile Dental Clinic','smile-dental-clinic','Advanced dental care with modern equipment.','Dental','+44 161 496 0213','khalid@smileclinic.com','52 Deansgate, Manchester M3 2BW']);
  for (const [n,d,c,t,p] of [
    ...
    ['Kids Dental Checkup','Friendly checkup for children','Pediatric',30,30],
  ]) await ins(`INSERT INTO services(business_id,name,description,category,duration_min,price) VALUES($1,$2,$3,$4,$5,$6) RETURNING id`,[s2,n,d,c,t,p]);
  await ins(`INSERT INTO staff(business_id,name,title,bio) VALUES($1,$2,$3,$4) RETURNING id`,[s2,'Dr. James Whitfield','Dental Surgeon','Expert in restorative and cosmetic dentistry.']);
  await ins(`INSERT INTO staff(business_id,name,title,bio) VALUES($1,$2,$3,$4) RETURNING id`,[s2,'Dr. Olivia Carter','Periodontist','Specialist in gum health and whitening.']);

  // Gym
  const go2 = await ins(`INSERT INTO users(name,email,password,role) VALUES($1,$2,$3,$4) RETURNING id`,
    ['Daniel Walker','ahmed@fitpro.com',h('owner123'),'business_owner']);
  const s3 = await ins(`INSERT INTO businesses(owner_id,name,slug,description,category,phone,email,address) VALUES($1,$2,$3,$4,$5,$6,$7,$8) RETURNING id`,
    [go2,'FitPro Gym','fitpro-gym','State-of-the-art fitness facility with certified trainers.','Fitness','+44 121 643 8820','ahmed@fitpro.com','78 Broad Street, Birmingham B1 2HF']);
  for (const [n,d,c,t,p] of [
    ['Personal Training','One-on-one with a certified trainer','Training',60,45],
    ['HIIT Class','High-intensity interval training','Classes',45,18],
    ['Yoga Session','Mindful yoga for flexibility','Classes',60,20],
    ['Strength Training','Guided weight training','Training',60,38],
    ['Cardio Blast Class','High-energy cardio group class','Classes',45,16],
    ['Pilates Session','Core-strengthening pilates','Classes',60,22],
    ['Nutrition Consultation','Personalised diet plan','Nutrition',45,35],
    ['Body Composition Test','Full BMI and body fat assessment','Assessment',30,25],
    ['Zumba Dance Class','Fun dance fitness class','Classes',60,15],
    ['Boxing Fitness','Boxing drills for strength','Training',60,30],
  ]) await ins(`INSERT INTO services(business_id,name,description,category,duration_min,price) VALUES($1,$2,$3,$4,$5,$6) RETURNING id`,[s3,n,d,c,t,p]);
  await ins(`INSERT INTO staff(business_id,name,title,bio) VALUES($1,$2,$3,$4) RETURNING id`,[s3,'Daniel Walker','Head Trainer','8 years in HIIT and functional fitness.']);
  await ins(`INSERT INTO staff(business_id,name,title,bio) VALUES($1,$2,$3,$4) RETURNING id`,[s3,'Emily Stevens','Yoga Coach','Specialised in yoga and pilates.']);

  // Photography
  const po2 = await ins(`INSERT INTO users(name,email,password,role) VALUES($1,$2,$3,$4) RETURNING id`,
    ['Charlotte Hughes','kamran@elitephoto.com',h('owner123'),'business_owner']);
  const s4 = await ins(`INSERT INTO businesses(owner_id,name,slug,description,category,phone,email,address) VALUES($1,$2,$3,$4,$5,$6,$7,$8) RETURNING id`,
    [po2,'Elite Photography Studio','elite-photography-studio','Professional photography for portraits, weddings and events.','Creative','+44 117 925 4471','kamran@elitephoto.com','9 Park Street, Bristol BS1 5NB']);
  for (const [n,d,c,t,p] of [
    ['Portrait Session','Studio portrait with professional lighting','Portrait',60,120],
    ['Wedding Photography','Full-day wedding coverage','Wedding',480,1200],
    ['Pre-Wedding Shoot','Romantic pre-wedding session','Wedding',180,350],
    ['Baby & Newborn Shoot','Safe newborn photography','Family',90,150],
    ...
    ['Product Photography','High-quality e-commerce photos','Commercial',120,250],
    ...
    ['Passport & CV Photos','Official passport photos same-day','Document',15,12],
  ]) await ins(`INSERT INTO services(business_id,name,description,category,duration_min,price) VALUES($1,$2,$3,$4,$5,$6) RETURNING id`,[s4,n,d,c,t,p]);
  await ins(`INSERT INTO staff(business_id,name,title,bio) VALUES($1,$2,$3,$4) RETURNING id`,[s4,'Charlotte Hughes','Lead Photographer','Award-winning, 12 years experience.']);

  // Wellness
  const wo2 = await ins(`INSERT INTO users(name,email,password,role) VALUES($1,$2,$3,$4) RETURNING id`,
    ['Dr. Hannah Cooper','farah@mindcarepk.com',h('owner123'),'business_owner']);
  const s5 = await ins(`INSERT INTO businesses(owner_id,name,slug,description,category,phone,email,address) VALUES($1,$2,$3,$4,$5,$6,$7,$8) RETURNING id`,
    [wo2,'MindCare Wellness Centre','mindcare-wellness','Holistic mental health and wellness services.','Wellness','+44 131 225 3344','farah@mindcarepk.com','3rd Floor, George Street, Edinburgh EH2 2PA']);
  for (const [n,d,c,t,p] of [
    ['Individual Therapy','One-on-one counselling session','Therapy',60,70],
    ['Couples Therapy','Joint counselling session','Therapy',90,95],
    ['Child Therapy','Therapy for children aged 5-16','Therapy',45,60],
    ['Anxiety Management','CBT-based anxiety session','Therapy',60,70],
    ['Depression Counselling','Supportive depression counselling','Therapy',60,70],
    ['Stress Management','Workplace stress techniques','Wellness',60,55],
    ['Guided Meditation','Mindfulness and meditation','Wellness',45,40],
    ['Sleep Therapy','Assessment for insomnia','Therapy',60,65],
    ['Life Coaching','Goal-setting and motivation','Coaching',60,60],
    ['Group Therapy Session','Group therapy for social anxiety','Therapy',90,30],
  ]) await ins(`INSERT INTO services(business_id,name,description,category,duration_min,price) VALUES($1,$2,$3,$4,$5,$6) RETURNING id`,[s5,n,d,c,t,p]);
  await ins(`INSERT INTO staff(business_id,name,title,bio) VALUES($1,$2,$3,$4) RETURNING id`,[s5,'Dr. Hannah Cooper','Clinical Psychologist','PhD, 10 years experience.']);
  await ins(`INSERT INTO staff(business_id,name,title,bio) VALUES($1,$2,$3,$4) RETURNING id`,[s5,'Andrew Fraser','Life Coach','CBT therapist specialising in anxiety.']);

  // Demo customer + appointments
  const cu = await ins(`INSERT INTO users(name,email,password,role,phone) VALUES($1,$2,$3,$4,$5) RETURNING id`,
    ['Jack Robinson','ali@customer.com',h('customer123'),'customer','+44 7700 900123']);
  await ins(`INSERT INTO appointments(business_id,service_id,customer_id,date,time,status,price) VALUES($1,$2,$3,$4,$5,$6,$7) RETURNING id`,[s1,1,cu,'2025-09-15','10:00','confirmed',35]);
  await ins(`INSERT INTO appointments(business_id,service_id,customer_id,date,time,status,price) VALUES($1,$2,$3,$4,$5,$6,$7) RETURNING id`,[s2,11,cu,'2025-09-20','11:00','pending',60]);
  await ins(`INSERT INTO appointments(business_id,service_id,customer_id,date,time,status,price) VALUES($1,$2,$3,$4,$5,$6,$7) RETURNING id`,[s3,21,cu,'2025-09-10','09:00','completed',45]);
  await ins(`INSERT INTO notifications(user_id,message) VALUES($1,$2) RETURNING id`,[cu,'Welcome to SmartBook! Browse businesses and book your first appointment.']);
  await ins(`INSERT INTO notifications(user_id,message) VALUES($1,$2) RETURNING id`,[so,'Welcome Sarah! Your Mayfair Hair Studio is now live on SmartBook.']);
  console.log('✅ Database seeded');
}

// ── JWT helpers ──────────────────────────────────────────────
const SECRET   = process.env.JWT_SECRET || 'smartbook-dev-secret';
const signTok  = p  => jwt.sign(p, SECRET, { expiresIn: '7d' });
const verifyTok = t => jwt.verify(t, SECRET);
...
// Ensure DB ready on every request
app.use(async (req, res, next) => {
  try { await ensureDB(); next(); }
  catch (e) { console.error('DB init error:', e.message); res.status(500).json({ error: 'Database error: ' + e.message }); }
});

function authenticate(req, res, next) {
  const h = req.headers.authorization;
  if (!h?.startsWith('Bearer ')) return res.status(401).json({ error: 'No token' });
  try { req.user = verifyTok(h.split(' ')[1]); next(); }
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    await dbRun('INSERT INTO notifications(user_id,message) VALUES($1,$2) RETURNING id',
      [userId, `Welcome to SmartBook, ${name}!`]);
    const user  = await dbGet('SELECT id,name,email,phone,role FROM users WHERE id=$1', [userId]);
    const token = signTok({ sub: userId, role: userRole, name: name.trim(), businessId });
...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    const token = signTok({ sub: user.id, role: user.role, name: user.name, businessId });
...
app.post('/api/services', authenticate, authorize('business_owner','super_admin'), async (req, res) => {
  try {
    ...
    if (!req.user.businessId) return res.status(400).json({ error: 'No business found' });
    const id = await dbRun('INSERT INTO services(business_id,name,description,category,duration_min,price) VALUES($1,$2,$3,$4,$5,$6) RETURNING id',
      [req.user.businessId,name.trim(),description||'',category||'General',parseInt(duration_min)||60,parseFloat(price)]);
...
app.get('/api/appointments/slots', async (req, res) => {
  try {
    ...
    const svc = await dbGet('SELECT * FROM services WHERE id=$1',[service_id]);
    if (!svc) return res.status(404).json({ error: 'Service not found' });
    const dur = parseInt(svc.duration_min)||60;
    const slots = [];
    let start = 9*60;
...
app.post('/api/appointments', authenticate, authorize('customer','super_admin'), async (req, res) => {
  try {
    ...
    business_id=parseInt(business_id); service_id=parseInt(service_id);
    ...
    const svc = await dbGet('SELECT * FROM services WHERE id=$1',[service_id]);
    const biz = await dbGet('SELECT * FROM businesses WHERE id=$1',[business_id]);
    if (!svc||!biz) return res.status(404).json({ error: 'Not found' });
    const apptId = await dbRun(`INSERT INTO appointments(business_id,service_id,staff_id,customer_id,date,time,notes,price) VALUES($1,$2,$3,$4,$5,$6,$7,$8) RETURNING id`,
      [business_id,service_id,staff_id,req.user.sub,date,time,notes||null,svc.price]);
    await dbRun('INSERT INTO notifications(user_id,message) VALUES($1,$2) RETURNING id',[biz.owner_id,`New booking: ${svc.name} on ${date} at ${time}`]);
    await dbRun('INSERT INTO notifications(user_id,message) VALUES($1,$2) RETURNING id',[req.user.sub,`Booking confirmed: ${svc.name} at ${biz.name} on ${date} at ${time}`]);
...
app.patch('/api/appointments/:id/status', authenticate, async (req, res) => {
  try {
    ...
    const msgs = {
      confirmed:`Your appointment for ${svc?.name} is confirmed!`,
      completed:`Your ${svc?.name} appointment is complete. Thank you!`,
      cancelled:`Your ${svc?.name} appointment has been cancelled.`
    };
...
app.get('/api/admin/stats', authenticate, authorize('super_admin'), async (req, res) => {
  try {
    ...
    res.json({totalUsers:users.c,totalBusinesses:businesses.c,totalAppointments:appts.c,
      totalRevenue:revenue.t,totalCustomers:customers.c,totalOwners:owners.c});
  } catch (e) { res.status(500).json({ error: e.message }); }
});
